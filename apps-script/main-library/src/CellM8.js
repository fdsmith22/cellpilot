...
const CellM8 = {
  ...
  SlideGenerator: {
    ...
    applyLightBackground: function(slide, theme) {
      ...
      if (theme.chartStyle !== 'minimal') {
        const overlay = slide.insertShape(
          SlidesApp.ShapeType.RECTANGLE,
          0, this.SLIDE_HEIGHT * 0.7,
          this.SLIDE_WIDTH, this.SLIDE_HEIGHT * 0.3
        );
    ...
    createPresentation: function(title, data, config) {
      try {
        ...
        Logger.log('Data structure prepared. Headers:', dataStructure.headers.length, 'Rows:', dataStructure.data.length);
    ...
    detectSemanticType: function(header, columnData) {
      ...
      if (headerLower.match(/salary|revenue|cost|price|amount|payment|budget|expense|income|profit|loss|fee|charge|total|subtotal|value|worth/)) {
        result.isMoney = true;
        result.semanticLabel = 'monetary';
      }
      
      // Detect performance/score columns
      if (headerLower.match(/score|rating|performance|grade|rank|percentile|efficiency|accuracy|quality|satisfaction|metric|index|kpi/)) {
        result.isPerformance = true;
        result.semanticLabel = 'performance';
      }
      
      // Detect person/employee columns
      if (headerLower.match(/employee|person|name|user|manager|supervisor|staff|worker|member|owner|customer|client|contact/)) {
        result.isPerson = true;
        result.semanticLabel = 'person';
      }
      
      // Detect location columns
      if (headerLower.match(/location|city|state|country|region|area|zone|district|address|site|office|branch|store|department|division/)) {
      ...
      // Detect date columns
      if (headerLower.match(/date|time|year|month|day|created|updated|modified|due|deadline|start|end|period|quarter/)) {
        result.isDate = true;
    ...
    createMetricCards: function(slide, metrics, theme, x, y, width, height) {
      ...
      metrics.slice(0, cardCount).forEach((metric, index) => {
        ...
        const shadow = slide.insertShape(
          ...
          cardX + 3, cardY + 3, cardWidth, cardHeight
        );
        ...
        const icon = metric.type === 'monetary' ? '💰' : 
                     metric.type === 'performance' ? '📈' : 
                     metric.type === 'average' ? '📊' : '🎯';
        
        const iconBox = slide.insertTextBox(
          icon,
          cardX + cardWidth/2 - 15, cardY + 15, 30, 30
        );
        iconBox.getText().getTextStyle()
          .setFontSize(24);
        iconBox.getText().getParagraphStyle()
        ...
        const valueBox = slide.insertTextBox(
          displayValue,
          cardX + 10, cardY + 50, cardWidth - 20, 40
        );
        ...
        const labelBox = slide.insertTextBox(
          metric.label,
          cardX + 10, cardY + 95, cardWidth - 20, 30
        );
      ...
      plan.forEach((slideSpec, index) => {
        ...
        switch(slideSpec.type) {
          ...
          case 'summary':
            ...
            this.addSpeakerNotes(slide, 
              'Executive summary showing key metrics.');
            break;
          case 'chart':
          ...
          case 'table':
            ...
            this.addSpeakerNotes(slide, 
              'Data table showing a sample of the dataset.');
            break;
          case 'insights':
            this.buildInsightsSlide(slide, slideSpec, theme);
            this.addSpeakerNotes(slide, 
              'Key insights derived from data analysis.');
            break;
          case 'conclusion':
    ...
    buildChartSlide: function(slide, spec, data, analysis, theme) {
      ...
      try {
        ...
        if (charts && charts.length > 0) {
          ...
          linkIndicator.getText().getTextStyle()
            .setFontSize(9)
            .setForegroundColor(theme.textLight)
            .setItalic(true);
    ...
    createSheetsChart: function(sheet, dataStructure, analysis, spec) {
      try {
        ...
        const chartBuilder = sheet.newChart()
          ...
          .addRange(dataRange)
          .setPosition(1, 10, 0, 0) // Place off to the side
          .setOption('title', spec.title || 'Data Analysis')
          .setOption('width', 600)
        ...
        chartBuilder
          .setOption('backgroundColor', currentTheme.background)
    ...
    selectChartDataRange: function(sheet, dataStructure, analysis) {
      try {
        ...
        if (analysis.suggestedVisualizations && analysis.suggestedVisualizations[0]) {
          const suggestion = analysis.suggestedVisualizations[0];
        ...
        return sheet.getRange(1, startCol, numRows, endCol);
    ...
    createVisualDataChart: function(slide, dataStructure, analysis, theme) {
      ...
      const dataArray = dataStructure && dataStructure.data ? dataStructure.data : 
                       Array.isArray(dataStructure) ? dataStructure : [];
      ...
      if (!dataArray || dataArray.length === 0) {
        ...
        noDataMsg.getText().getTextStyle()
          .setFontSize(14)
          .setForegroundColor(theme.textLight)
          .setItalic(true);
        noDataMsg.getText().getParagraphStyle()
          .setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER);
        return;
      }
      ...
      if (analysis.suggestedVisualizations && analysis.suggestedVisualizations.length > 0) {
        ...
        switch (topSuggestion.type) {
          case 'bar_chart':
          case 'grouped_bar':
            this.createIntelligentBarChart(slide, displayData, analysis, topSuggestion, theme, chartX, startY, chartWidth, chartHeight);
            break;
          case 'line_chart':
            this.createTimeSeriesChart(slide, displayData, analysis, topSuggestion, theme, chartX, startY, chartWidth, chartHeight);
            break;
          case 'pie_chart':
            this.createDistributionChart(slide, displayData, analysis, topSuggestion, theme, chartX, startY, chartWidth, chartHeight);
            break;
          case 'metric_cards':
          ...
          default:
            ...
            this.createBarChartVisualization(slide, displayData, analysis, theme, chartX, startY, chartWidth, chartHeight);
        }
        ...
        if (topSuggestion.reason) {
          ...
          reasonBox.getText().getTextStyle()
            .setFontSize(10)
            .setForegroundColor(theme.textLight)
            .setItalic(true);
        }
        ...
        if (hasNumeric && displayData.length > 0) {
          ...
          this.createPercentageVisualization(slide, displayData, analysis, theme, chartX, startY, chartWidth, chartHeight);
        } else {
    ...
    createIntelligentBarChart: function(slide, data, analysis, suggestion, theme, x, y, width, height) {
      ...
      // Create axis
      const yAxis = slide.insertShape(
        SlidesApp.ShapeType.RECTANGLE,
        x + 40, y + 30, 1, maxBarHeight
      );
      yAxis.getFill().setSolidFill(theme.textLight);
      yAxis.getBorder().setTransparent();
      
      const xAxis = slide.insertShape(
        SlidesApp.ShapeType.RECTANGLE,
        x + 40, y + 30 + maxBarHeight, width - 80, 1
      );
      ...
      sortedData.forEach((item, index) => {
        ...
        if (barHeight > 2) {
          const shadow = slide.insertShape(
            SlidesApp.ShapeType.RECTANGLE,
            barX + 2, barY + 2, barWidth, barHeight
          );
          shadow.getFill().setSolidFill('#e0e0e0');
          shadow.getBorder().setTransparent();
          shadow.sendToBack();
        ...
        const valueLabel = slide.insertTextBox(
          valueText,
          barX, barY - 25, barWidth, 20
        );
        valueLabel.getText().getTextStyle()
          .setFontSize(10)
          .setForegroundColor(barColor)
          .setBold(true);
        valueLabel.getText().getParagraphStyle()
          .setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER);
        
        // Category label
        const categoryLabel = slide.insertTextBox(
          item.category ? item.category.toString().substring(0, 10) : 'Unknown',
          barX - 5, y + 30 + maxBarHeight + 5, barWidth + 10, 30
        );
        categoryLabel.getText().getTextStyle()
          .setFontSize(9)
          .setForegroundColor(theme.textLight);
        categoryLabel.getText().getParagraphStyle()
          .setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER);
        ...
        if (item.count > 1) {
          const countLabel = slide.insertTextBox(
            '(n=' + item.count + ')',
            barX, y + 30 + maxBarHeight + 30, barWidth, 15
          );
          countLabel.getText().getTextStyle()
            .setFontSize(8)
            .setForegroundColor(theme.textLight)
            .setItalic(true);
          countLabel.getText().getParagraphStyle()
            .setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER);
        }
      });
      ...
      const xLabel = slide.insertTextBox(
        ...
        x + width/2 - 50, y + height - 20, 100, 20
      );
      xLabel.getText().getTextStyle()
        .setFontSize(10)
        .setForegroundColor(theme.textLight);
      xLabel.getText().getParagraphStyle()
        .setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER);
    },
    ...
    createBarChartVisualization: function(slide, data, analysis, theme, x, y, width, height) {
      ...
      values.slice(0, barCount).forEach((value, index) => {
        ...
        // Category label
        const categoryText = data[index][0] ? data[index][0].toString().substring(0, 8) : '';
        ...
        label.getText().getTextStyle()
          .setFontSize(9)
          .setForegroundColor(theme.textLight);
        label.getText().getParagraphStyle()
          .setParagraphAlignment(SlidesApp.ParagraphAlignment.CENTER);
      });
      
      // Add axis labels
      const yLabel = slide.insertTextBox(
        analysis.headers[numericCol] || 'Values',
        x,
        y + height/2 - 20,
        25,
        40
    ...
    createPercentageVisualization: function(slide, data, analysis, theme, x, y, width, height) {
      ...
      dataArray.slice(0, maxRows).forEach((row, index) => {
        ...
        // Progress bar fill
        if (barWidth > 0) {
          const barFill = slide.insertShape(
            SlidesApp.ShapeType.ROUND_RECTANGLE,
            x + 170,
            rowY + 5,
            barWidth,
        ...
        // Percentage label
        const percentLabel = slide.insertTextBox(
        ...
        percentLabel.getText().getTextStyle()
          .setFontSize(10)
          .setForegroundColor(theme.textLight)
          .setBold(true);
        percentLabel.getText().getParagraphStyle()
          .setParagraphAlignment(SlidesApp.ParagraphAlignment.END);
      });
    },
    ...
    createDataCards: function(slide, data, analysis, theme, x, y, width, height) {
      ...
      data.slice(0, cardCount).forEach((row, index) => {
        ...
        // Card accent bar
        const accentBar = slide.insertShape(
          SlidesApp.ShapeType.RECTANGLE,
          cardX,
          cardY,
          cardWidth,
  ...
  addMetricsToZone: function(slide, zone, metrics, theme) {
    ...
    metrics.slice(0, cardCount).forEach((metric, index) => {
      ...
      const card = slide.insertShape(
        SlidesApp.ShapeType.ROUND_RECTANGLE,
        cardX, zone.position.y, cardWidth, zone.position.h
      );
      card.getFill().setSolidFill(theme.surface || '#F8F9FA');
      card.getBorder().getLineFill().setSolidFill(theme.primary || '#1B2951');
      card.getBorder().setWeight(1);
      ...
      const valueBox = slide.insertTextBox(
        valueText,
        cardX + 5, zone.position.y + 10, cardWidth - 10, zone.position.h * 0.5
      );
      valueBox.getText().getTextStyle()
        .setFontSize(20)
        .setBold(true)
      ...
      const labelBox = slide.insertTextBox(
        metric.label,
        cardX + 5, zone.position.y + zone.position.h * 0.6, cardWidth - 10, zone.position.h * 0.3
      );
      labelBox.getText().getTextStyle()
        .setFontSize(10)
